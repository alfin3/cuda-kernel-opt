...
//
// This highly portable implementation may be used across industry-grade GPUs and provide an informed
...
// tuning the matrix multiply and accumulate workload of a warp, and changing the following parameters:
// DT: half,
...
void gemm_shmem_tc_async_opt_port(
    ...
    if (num_acc == 2) {
        gemm_shmem_tc_async_opt_port_kernel<DT, DT_ACC, 2><<<gridDim, blockDim, shmem_req>>>(A, B, C, D, alpha, beta,
            M, N, K, tile_dim, segment_dim_k, tile_group_m, num_stages, num_producer_warps, num_consumer_warps);
    }
    if (num_acc == 4) {
                                ...
                                if (segment_dim_k <= tile_dim) {
                                    for (int i = 0; i < num_reps; ++i) {
                                        ...
                                        gemm_shmem_tc_async_opt_port<DT, DT_ACC>(A, B, C, D, alpha, beta,
                                            dim.m, dim.n, dim.k, tile_dim, segment_dim_k, tile_group_m, num_stages,
                                            num_producer_warps, num_consumer_warps, prop->multiProcessorCount);
                                        checkCuda(cudaMemcpy(mt.GetRes(), D, mem_size_d, cudaMemcpyDeviceToHost));
                                        res = res && mt.IsCorrect(mt.GetRes(), dim, alpha, beta, atol);
                                    }
                                    printf("(%d, %d, %d, %d, %d, %d): %s\n", tile_dim, segment_dim_k, tile_group_m, num_stages,
...
int main(void) {
    ...
    RunCorrectnessTestSquare<half, half>(
        &prop, 1.0f, 1.0f, M, N, K, 64, 128, 32, 256, 1, 6, 1, 10, 1, 8, 4, 8, 2, 0.001f);
    ...
    RunAccuracyTestSquare<half, half>(
        &prop, 1.0f, 1.0f, M, N, K, 64, 128, 32, 256, 2, 6, 1, 4, 1, 8, 4, 8, 2, -1.0f, 1.0f, 0.1f);
