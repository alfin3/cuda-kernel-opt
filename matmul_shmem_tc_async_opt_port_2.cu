...
//
// An optimization was implemented with respect to the synchronization scheme. Instead of providing
// barriers for every consumer warp, or every row and column of a warp tile, it may be beneficial to
// only provide separate barriers for the first few upper left consumer warps or the first few upper
// left rows and columns of a warp tile and to handle the residual consumer warps or rows and columns
// of a warp tile with single barriers. According to this optimization, an early start of matrix
// multiply and accumulate may be achieved, but with fewer barriers and decreased synchronization
// overhead. Additionally, the ability to handle the rest of the warp tile with arrays of fragments
// is preserved.
//
                ...
                if (bar_step_rows_a == rows) {
                    ...
                    if (i % warp_tile_rows <= bar_steps_a) {
                        for (int j = 0; j < 2; ++j) { // rows / (warp_tile_cols * WMMA_N) == 2.
                            ...
                            empty[(segment_count % num_stages) * num_consumer_warps * (bar_steps_a + 1) +
                                consumer_warp_id * (bar_steps_a + 1) + (i % warp_tile_rows)].arrive_and_wait();
                        }
                    }
                }
...
#pragma unroll
                ...
                if (bar_step_rows_b == rows) {
                    ...
                    if (i % warp_tile_cols <= bar_steps_b) {
                        ...
                        for (int j = 0; j < num_consumer_warps; j += 2) {
                            ...
                            empty[bar_offset_b + (segment_count % num_stages) * num_consumer_warps * (bar_steps_b + 1) +
                                consumer_warp_id * (bar_steps_b + 1) + (i % warp_tile_cols)].arrive_and_wait();
                        }
                    }
                }
    ...
    if (producer_warp_id < num_producer_warps / 2) {
        ...
        if (bar_step_rows_b == rows) {
            ...
            const int bar_offset_b = num_stages * num_consumer_warps * (bar_steps_a + 1);
            for (int i = 0; i < num_stages * num_consumer_warps * (bar_steps_b + 1); ++i) {
                auto token = empty[bar_offset_b + i].arrive();
            }
        }
    }
...
void consume(
    ...
    for (int i = 0; i < num_stages; ++i) {
        if (bar_step_rows_a == rows && bar_step_rows_b == rows) {
            ...
            const int bar_offset_b = num_stages * (bar_steps_a * bar_steps_b + 1);
            int bar_id = bar_steps_a * bar_steps_b;
            if (consumer_warp_id / 2 < bar_steps_a && consumer_warp_id % 2 < bar_steps_b) {
                bar_id = (consumer_warp_id / 2) * bar_steps_b + consumer_warp_id % 2;
            }
            auto token_a = empty[i * (bar_steps_a * bar_steps_b + 1) + bar_id].arrive();
            auto token_b = empty[bar_offset_b + i * (bar_steps_a * bar_steps_b + 1) + bar_id].arrive();
        } else if (bar_step_rows_a == WMMA_M && bar_step_rows_b == WMMA_N) {
            for (int j = 0; j < (bar_steps_a + 1); ++j) {
                auto token_a = empty[i * num_consumer_warps * (bar_steps_a + 1) +
                    consumer_warp_id * (bar_steps_a + 1) + j].arrive();
            }
            const int bar_offset_b = num_stages * num_consumer_warps * (bar_steps_a + 1);
            for (int j = 0; j < (bar_steps_b + 1); ++j) {
                auto token_b = empty[bar_offset_b + i * num_consumer_warps * (bar_steps_b + 1) +
                    consumer_warp_id * (bar_steps_b + 1) + j].arrive();
            }
        }
    }
    ...
    for (int segment_count = 0; segment_count < K / cols; ++segment_count) {
        ...
        if (bar_step_rows_a == WMMA_M && bar_step_rows_b == WMMA_N){
            ...
            // Compute all rows against first the first few columns of accumulators, prioritizing the
            // upper left region of rows and columns of accumulators.
#pragma unroll
            for (int i = 0; i < warp_tile_rows; ++i) {
                const int shmem_row_a = (consumer_warp_id / 2) * warp_tile_rows * WMMA_M + i * WMMA_M;
                if (i <= bar_steps_a) {
                    full[(segment_count % num_stages) * num_consumer_warps * (bar_steps_a + 1) +
                        consumer_warp_id * (bar_steps_a + 1) + i].arrive_and_wait();
                }
#pragma unroll
                for (int j = 0; j < bar_steps_b; ++j) {
                    const int shmem_row_b = (consumer_warp_id % 2) * warp_tile_cols * WMMA_N + j * WMMA_N;
                    if (i == 0) {
                        full[bar_offset_b + (segment_count % num_stages) * num_consumer_warps * (bar_steps_b + 1) +
                            consumer_warp_id * (bar_steps_b + 1) + j].arrive_and_wait();
                    }
                    ...
                    if (i == warp_tile_rows - 1) { // Not reached if bar_steps_b == 0.
            ...
            // Compute all rows against the residual columns of accumulators.
#pragma unroll
            for (int i = 0; i < warp_tile_rows; ++i) {
                const int shmem_row_a = (consumer_warp_id / 2) * warp_tile_rows * WMMA_M + i * WMMA_M;
                if (i == 0) { // If bar_steps_b == 0, this is the residual barrier for B.
                    full[bar_offset_b + (segment_count % num_stages) * num_consumer_warps * (bar_steps_b + 1) +
                        consumer_warp_id * (bar_steps_b + 1) + bar_steps_b].arrive_and_wait();
                }
#pragma unroll
                for (int j = bar_steps_b; j < warp_tile_cols; ++j) {
                    const int shmem_row_b = (consumer_warp_id % 2) * warp_tile_cols * WMMA_N + j * WMMA_N;
                    consume_acc<DT, DT_ACC>(&warp_tile_c[i * warp_tile_cols + j], shmem_ptr, K, rows, cols,
                        shmem_row_a, shmem_row_b, shmem_stride, num_stages, (segment_count % num_stages));
                }
                ...
                if (i < bar_steps_a) {
                    auto token_a = empty[(segment_count % num_stages) * num_consumer_warps * (bar_steps_a + 1) +
                        consumer_warp_id * (bar_steps_a + 1) + i].arrive();
                }
            ...
            if (bar_step_rows_a == rows && bar_step_rows_b == rows) {
                ...
                full[(segment_count % num_stages) * (bar_steps_a * bar_steps_b + 1) +
                    bar_id].arrive_and_wait();
                full[bar_offset_b + (segment_count % num_stages) * (bar_steps_a * bar_steps_b + 1) +
                    bar_id].arrive_and_wait();
            }
...
#pragma unroll
            ...
            if (bar_step_rows_a == rows && bar_step_rows_b == rows) {
                ...
                auto token_a = empty[(segment_count % num_stages) * (bar_steps_a * bar_steps_b + 1) +
                    bar_id].arrive();
                auto token_b = empty[bar_offset_b + (segment_count % num_stages) * (bar_steps_a * bar_steps_b + 1) +
                    bar_id].arrive();
            }
        }
    }
...
void gemm_shmem_tc_async_opt_port_kernel(
    ...
    // The empty barriers for A across all stages are followed by the empty barriers for B across
    // all stages. The empty barriers are followed by full barriers. The bar_steps_{a, b} and
    // bar_step_rows_{a, b} parameter values result in the following mapping:
    //     bar_step_rows_a == tile_dim && bar_step_rows_b == tile_dim:
    //         all consumer warps are mapped to one empty barrier per stage for A and one empty barrier
    //         per stage for B; the layout of barriers corresponds to the consumer warps, and the
    //         values of bar_steps_{a, b} have no effect,
    //     bar_step_rows_a == warp_tile_rows * WMMA_M && bar_step_rows_b == warp_tile_cols * WMMA_N:
    //         a consumer warp within the upper left region determined by bar_steps_{a, b}, is
    //         mapped to one empty barrier per stage for A and one empty barrier per stage for B;
    //         all other consumer warps are mapped to a single empty barrier per stage for A and a
    //         single empty barrier per stage for B; barriers are in row-major order according to
    //         the values of bar_steps_{a, b},
    //     bar_step_rows_a == WMMA_M && bar_step_rows_b == WMMA_N:
    //         a consumer warp is mapped to bar_steps_a empty barriers per stage for A and bar_steps_b
    //         empty barriers per stage for B, and two additional empty barriers per stage for the
    //         residual rows and columns of the warp tile that are not within the upper left region
    //         determined by the values of bar_steps_{a, b} .
    ...
    int num_bars_empty_a, num_bars_empty_b;
    if (bar_step_rows_a == tile_dim && bar_step_rows_b == tile_dim) {
    ...
    if (threadIdx.x >= num_producer_warps * WARP_SIZE &&
        ...
        if (bar_step_rows_a == tile_dim) {
            ...
            const int bar_thread_count_a_residual = (num_producer_warps / 2 + (num_consumer_warps - bar_steps_a * bar_steps_b)) * WARP_SIZE;
            for (int i = 0; i < num_stages; ++i) {
    ...
    if (threadIdx.x >= num_producer_warps * WARP_SIZE &&
        ...
        if (bar_step_rows_b == tile_dim) {
            ...
            const int bar_thread_count_b_residual = (num_producer_warps / 2 + (num_consumer_warps - bar_steps_a * bar_steps_b)) * WARP_SIZE;
            for (int i = 0; i < num_stages; ++i) {
...
void gemm_shmem_tc_async_opt_port(
    ...
    const int shmem_req = get_shmem_req<DT, DT_ACC>(tile_dim, segment_dim_k, bar_steps_a, bar_steps_b,
        bar_step_rows_a, bar_step_rows_b, num_stages, num_consumer_warps);
    const int num_acc = get_num_acc(tile_dim, num_consumer_warps);
    dim3 gridDim(num_sms, 1, 1);
    dim3 blockDim((num_producer_warps + num_consumer_warps) * WARP_SIZE, 1, 1);
    if (shmem_req + SHMEM_ALIGNMENT > 48 * 1024) {
        if (num_acc == 2) {
    ...
    if (num_acc == 2) {
        gemm_shmem_tc_async_opt_port_kernel<DT, DT_ACC, 2><<<gridDim, blockDim, shmem_req>>>(A, B, C, D, alpha, beta,
            M, N, K, tile_dim, segment_dim_k, bar_steps_a, bar_steps_b, bar_step_rows_a, bar_step_rows_b, tile_group_m,
            num_stages, num_producer_warps, num_consumer_warps);
    }
    if (num_acc == 4) {
                                            ...
                                            if (segment_dim_k <= tile_dim) {
                                                for (int j = 0; j < num_reps; ++j) {
                                                    ...
                                                    gemm_shmem_tc_async_opt_port<DT, DT_ACC>(A, B, C, D, alpha, beta,
                                                        dim.m, dim.n, dim.k, tile_dim, segment_dim_k, bar_steps_a,
                                                        bar_steps_b, bar_step_rows_a[i], bar_step_rows_b[i], tile_group_m,
                                                        num_stages, num_producer_warps, num_consumer_warps, num_sms);
                                    ...
                                    for (int bar_steps_b = 0; bar_steps_b < bar_steps_b_end[i]; ++bar_steps_b) {
                                        if (get_shmem_req<DT, DT_ACC>(tile_dim, segment_dim_k, bar_steps_a, bar_steps_b,
                                            ...
                                            if (segment_dim_k <= tile_dim) {
                                                ...
                                                printf("(%d, %d, %d, %d, %d, %d, %d, %d, %d, %d): %.2f / %d\n", tile_dim,
                                                    segment_dim_k, bar_steps_a, bar_steps_b, bar_step_rows_a[i],
                                                    bar_step_rows_b[i], tile_group_m, num_stages, num_producer_warps,
